...
Voraussichtlich im November/Dezember 2026 wird ein Abzug der SVN-Repositories
nach Git kopiert. Dieser Stand dient dem Testbetrieb. Jenkins und SVN bleiben
weiterhin produktiv. Der Teststand wird nicht laufend mit SVN abgeglichen.

Für den ab Januar 2027 geplanten Cutover wird der freigegebene SVN-Stand nach
Git übertragen und geprüft. Während der Umschaltung liefert nur eines der
beiden Systeme produktiv.
...
| offen | Projektmatrix inventarisieren und freigeben | Projektverzeichnisse, Ausschlüsse und historisch festgelegte Lieferdateinamen sind je Mandant und gegebenenfalls je Releaselinie dokumentiert |
...
| offen | Projektausschlüsse festlegen | Testdatenprojekte wie `LOMS_Testdaten` und `LOMS_Testdaten_Autonom` können im Repository liegen und werden über `excluded_projects` von der Auslieferung ausgeschlossen |
...
3. Inhalte, Pfadabbildungen, Tags und den letzten migrierten Commit je Mandant
   dokumentieren und automatisch prüfen.
...
- Ressourcenbereitstellung, Adapterantwort, Artefaktprüfung, JCL und die
  unmittelbare Mainframe-Übergabe sind erfolgreich geprüft.
